 ...
 */
// cf. https://github.com/jawj/mtwist
...
class MTwist {
    ...
    randomIntBelow(maxPlusOne) {
        ...
        if (maxPlusOne === 1)
            return 0;
        const bitsNeeded = Math.ceil(Math.log2(maxPlusOne)), bitMask = (1 << bitsNeeded) - 1;
        while (true) {
    ...
    static test() {
        ...
        for (let i = 0; i < 1000; i++) {
            const mtwist = new MTwist(seed), iterations = Math.floor(mtwist.randomUInt32() / iterationFactor);
...
singleton.do = new MTwist(singleton.seed)
singleton.set_seed = (seed) => {
    singleton.seed = seed
    singleton.do = new MTwist(seed)
}
